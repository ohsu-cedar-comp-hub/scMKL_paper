...
def tfidf_train_test(X_train, X_test):
    ...
    '''
    Function to calculate TFIDF normalization for split training and testing data.
    Calculates inverse document frequency on train data and uses as a 'parameter' to
        normalize test data.

    Input:
        X_train, X_test- Numpy/Scipy sparse arrays containing train/test data respectively
    Output:
        tfidf_train, tfidf_test- Numpy/Scipy sparse arrays containing normalized
                                    train/test data respectively.
    '''

    # Use atac data as the term frequency
    # Calculate document frequency as the count of non zero elements in each row
    if scipy.sparse.issparse(X_train):
        # Converts to sparse array for compatability with numpy
    ...
    # Uses document frequency to calculate inverse document frequency
...
output_dir = f'/home/groups/CEDAR/scMKL/results/XGBoost/{dataset}/{assay}/{subset}'
...
if subset == 'hallmark':
    # Filter out features not in hallmark
    ...
    # Removes all but the top 5000 most variable features in a dataset.
...
# Performs 4 fold cross validation over values for max depth
...
for fold in np.arange(k):
    # Creates indepedent validation train/test sets
    ...
    if subset == 'dimension_reduction':
        # This must be calculated separately for each fold to avoid data leakage to validation test data
        if assay in ['rna', 'gene_scores']:
            # Calculates 50 component PCA for rna/gene scores
            ...
            # Calculates 50 component SVD on tfidf normalized atac data- Latent Semantic Indexing (LSI).
            ...
            # Calculates top 50 components but removes the top component as this is correlated with
            # sequencing depth
    ...
    for i, depth in enumerate(max_depths):
        model = HistGradientBoostingClassifier(max_depth= depth, early_stopping= True, tol = 0.0001).fit(fold_train_X, y_train[fold_train])
...
# Finds the depth parameter that maximized validation auroc
...
model = HistGradientBoostingClassifier(max_depth= opt_depth, early_stopping= True, tol = 0.0001).fit(X_train, y_train)
