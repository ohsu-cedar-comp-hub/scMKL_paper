...
parser.add_argument('-s', '--feature_subset', help = 'Which_features to subset the data from', choices = [None, 'hallmark', 'mvf', 'dimension_reduction'])
parser.add_argument('-k', '--kernel_function', help = 'Kernel function to use', choices = ['rbf', 'linear'])
...
output_dir = f'/home/groups/CEDAR/scMKL/results/SVM/{dataset}/{assay}/{subset}/{kernel_function}'
...
print(f'{tracemalloc.get_traced_memory()[1] / 1e9} GB')
...
for fold in np.arange(k):
    ...
    if subset == 'dimension_reduction':
        if assay in ['rna', 'gene_scores']:
            ...
            fold_train_X, fold_test_X = Process_Data(fold_train_X, fold_test_X, data_type = datatype)
...
if subset == 'dimension_reduction':
    ...
    X_train, X_test = Process_Data(X_train, X_test, data_type = 'counts')
