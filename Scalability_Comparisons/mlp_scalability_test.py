...
parser.add_argument('-a', '--assay', help = 'Which assay to run with', type = str, choices = ['rna', 'atac'])
...
output_dir = f'/home/groups/CEDAR/scMKL/results/scalability_tests/{assay}/MLP/{subset}/{n_cells}'
...
batch_sizes = np.power(2, np.arange(len(bin(len(train_indices))[2:]) - 5, len(bin(len(train_indices))[2:]))) # sets 5 batch sizes smaller than the train set
k = 4
...
for fold in np.arange(k):
    ...
    for n in np.arange(len(batch_sizes)):
        ...
        history = model.fit(fold_train_X, y_train[fold_train], batch_size= batch_sizes[n], epochs= 10, validation_split=0.0, callbacks = [callback], verbose = 0)
...
start = time.time()
tracemalloc.start()
# tf.profiler.profile(options = tf.profiler.ProfileOptionBuilder.time_and_memory())
...
print(f'Tracemalloc: {tracemalloc.get_traced_memory()[0] / 1e9} GB')
